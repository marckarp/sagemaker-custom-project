...
def retrieve_model_group(sagemaker_project_id):
    
    s3 = boto3.client('s3')
    bucket_name = f'sagemaker-project-{sagemaker_project_id}' 
    key = 'model-group/model-package-group-name.json' 
    
    # Download the JSON file from S3
    s3.download_file(bucket_name, key, '/tmp/sagemaker-event.json')
    
    # Load the JSON file
    with open('/tmp/sagemaker-event.json') as f:
        data = json.load(f)
    
    # Retrieve the value of SOURCE_MODEL_PACKAGE_GROUP_NAME
    source_model_package_group_name = data['SOURCE_MODEL_PACKAGE_GROUP_NAME']
    
    print(f"Deploying mode from Model Group: {source_model_package_group_name}")
    
    return source_model_package_group_name
...
if __name__ == "__main__":
    ...
    model_package_arn = get_approved_package(retrieve_model_group(args.sagemaker_project_id))
